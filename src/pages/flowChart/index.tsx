...
const Wrap: React.FC = () => {
  const logicflowRef = useRef<any>(null);
  ...
  const init = () => {
    const logicflow = new LogicFlow({
      ...
      plugins: [Menu, MiniMap, DndPanel, SelectionSelect, Control, Snapshot],
      snapline: true,
    });
    ...
    logicflow.render({
      nodes: [
        { id: "1", type: "rect", x: 400, y: 100, text: "音频浓缩" },
        { id: "2", type: "diamond", x: 300, y: 200, text: "神经网络降噪" },
      ],
      edges: [
        { sourceNodeId: "1", targetNodeId: "2", type: "polyline", text: "11" },
      ],
    });
    ...
    logicflow.zoom(1);
    // 显示缩略图
    logicflow.extension.miniMap.show(500, 300);
  ...
  const downloadImage = async (bgColor = "#fff") => {
    if (logicflowRef.current) {
      const snapshot: Snapshot = logicflowRef.current.extension.snapshot;
  ...
  return (
    <div style={{ width: "100%", height: "100%", padding: 20 }}>
      <h2 onClick={add}>流程图</h2>
      <h2 onClick={() => downloadImage()}>下载</h2>
